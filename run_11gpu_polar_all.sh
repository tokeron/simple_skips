#!/bin/bash
#
# Submit 11 single-GPU jobs on the polar partition to run MAIN and ANALYSIS
# over the sample CSV with 100 seeds. Each job handles a disjoint slice of
# CSV rows and runs both scripts sequentially on its allocated GPU.
#
...
# Variants for MAIN: choose 'specific' (recommended for speed) or 'all'
MAIN_GENERATE_VARIANTS=${MAIN_GENERATE_VARIANTS:-specific}
NUM_JOBS=${NUM_JOBS:-11}
...
RUN_LOG_DIR="${LOG_DIR_BASE}/flux_dev_polar_${TIMESTAMP}"
mkdir -p "${RUN_LOG_DIR}"

echo "Submitting ${NUM_JOBS} single-GPU jobs to polar at $(date)"
...
for job_idx in $(seq 0 $((NUM_JOBS-1))); do
  ...
  job_name="fluxdev_polar_${job_idx}_${start}_${end}"
  ...
  sbatch \
    ...
    --job-name="${job_name}" \
    --output="${sbatch_out}" \
    --wrap "set -euo pipefail; echo \"Job ${job_name} starting on $(hostname) at $(date)\"; export PYTHONUNBUFFERED=1; ${PYTHON_PATH} ${SCRIPT_DIR}/run_single_gpu.py --model_name ${MODEL_NAME} --prompts_csv ${PROMPTS_CSV} --generate_variants ${MAIN_GENERATE_VARIANTS} --seeds ${SEEDS} --start_idx ${start} --end_idx ${end}; ${PYTHON_PATH} ${SCRIPT_DIR}/run_single_gpu.py --analysis --model_name ${MODEL_NAME} --prompts_csv ${PROMPTS_CSV} --seeds ${SEEDS} --generate_original_image --start_idx ${start} --end_idx ${end}; echo \"Job ${job_name} completed at $(date)\";"

  start=${end}
done

echo "All jobs submitted. Monitor with: tail -f ${RUN_LOG_DIR}/*.out"
